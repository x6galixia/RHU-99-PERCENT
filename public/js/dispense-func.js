document.addEventListener("DOMContentLoaded", function () {
    ...
    // Set the current date in the 'date_issued' input field
    document.getElementById("date_issued").value = getCurrentDate();

    // Function to fill the input fields with the provided data
    function fillInputs(
        full_name,
        gender,
        full_address,
    ...
    // Attach click handlers to each row in the table
    ...
    // Handle input in the 'product_details' field
        ...
        if (query.length > 0) {
            fetch(`/get?query=${encodeURIComponent(query)}`)
                ...
                .then(data => {
                    ...
                    if (data && data.expiration_date && data.batch_number) {
